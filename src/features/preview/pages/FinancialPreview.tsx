...
export default function FinancialPreview() {
  ...
  return (
    <PreviewDocument>
      <PreviewBlock id="financial-main">
        <div>
          ...
          <div className="flex flex-col gap-6">
            {CHARGE_CATEGORIES.map((category) => {
              if (category === "Food & Beverage") {
                if (!hasFoodBeverageSection) return null
                return (
                  <ChargeTable key={category} title="Food & Beverage">
              ...
              return (
                <ChargeTable key={category} title={category}>
                  {items.map((item) => (
                    <ChargeRow
                      key={item.id}
                      name={item.name}
            ...
            {(chargeItemsByCategory["Other"]?.length ?? 0) > 0 ? (
              <ChargeTable title="Other">
      ...
      {showAppendix ? (
        <PreviewBlock id="financial-beverage-appendix" breakBefore>
          <div>
