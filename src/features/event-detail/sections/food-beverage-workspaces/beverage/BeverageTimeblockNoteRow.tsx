  ...
  return (
    <div className="flex items-start py-1">
      ...
      {isEditing ? (
        ...
        <div
          className={`${shortNote.length <= 0 ? "text-stone-400" : "text-stone-500"} m-0.25 min-h-7.5 w-full px-1.5 py-1`}
          onClick={handleStartEdit}
        >
          {shortNote.length <= 0 ? "Notes..." : shortNote}
        </div>
      )}
    </div>
  )
